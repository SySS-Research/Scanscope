...
from bokeh.models import HoverTool, ColumnDataSource, CategoricalColorMapper, WheelZoomTool, CustomJS, Select
...
def plot(data, filename, circle_scale=1, title=None, **kwargs):
    ...
    color_mapping = CategoricalColorMapper(factors=["%02x" % x for x in range(256)],
    ...
    callback_click = CustomJS(
        args=dict(datasource=datasource, fp_map=data['fp_map'], color_map=color_mapping),
...
def reduce_and_plot(data, filename, circle_scale=1, title="", post_deduplicate=True, **kwargs):
