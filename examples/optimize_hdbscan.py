...
Note: This is the same as optimize_umap.py since scanscope uses the same
unified approach for both clustering and projection optimization.

Installation:
    uv sync --group optimize

Usage:
    uv run examples/optimize_hdbscan.py nmap_output.xml --trials 50

For parallel execution:
    uv run examples/optimize_hdbscan.py nmap_output.xml --trials 50 --jobs 4

With PCA projection (faster, no extra parameters to tune):
    uv run examples/optimize_hdbscan.py nmap_output.xml --trials 50 --projection pca

With clustering on 2D projection (may find more generalizable parameters):
    uv run examples/optimize_hdbscan.py nmap_output.xml --trials 50 --cluster-on projection
"""
...
def main() -> None:
    ...
    parser.add_argument("--pre-deduplicate", action="store_true", help="Deduplicate before clustering")
    parser.add_argument("--remove-empty", action="store_true", help="Remove hosts with no open ports")
    parser.add_argument(
        "--extended-search",
        action="store_true",
        help="Enable extended parameter search (wider ranges, more parameters)",
    )
    parser.add_argument(
        "--projection",
        type=str,
    ...
    parser.add_argument(
        ...
        help="What to cluster on: 'original' (high-dim data, more accurate) or 'projection' "
        "(2D, may generalize better)",
    )
